...
export const postSchema = ({ image }: SchemaContext) =>
  z.object({
    ...
    titleIcon: z
      .union([
        z.url().describe('Image URL for the title icon.'),
        z
          .string()
          .regex(
            /^(i-)?[\w-]+(:[\w-]+)?$/,
            'Icon must be in the format `<collection>-<icon>`, `<collection>:<icon>`, or with `i-` prefix as per UnoCSS icon specs.'
          )
          .describe(
            'UnoCSS icon in the format `<collection>-<icon>`, `<collection>:<icon>`, or with `i-` prefix.'
          ),
      ])
      .optional()
      .describe(
...
export const projectSchema = z.object({
  ...
  icon: z
    .union([
      z.url().describe('Image URL for the project icon.'),
      z
        .string()
        .regex(
          /^(i-)?[\w-]+(:[\w-]+)?$/,
