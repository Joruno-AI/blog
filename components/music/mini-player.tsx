      ...
      <AnimatePresence mode="wait">
        {isExpanded ? (
          <motion.aside key="expanded" initial={{ opacity: 0, scale: .94, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: .94, y: 20 }} className="fixed bottom-6 right-6 z-50 w-80 overflow-hidden rounded-2xl border bg-card shadow-2xl" aria-label="音乐播放器">
            <div className="absolute right-3 top-3 z-10 flex gap-1"><Button variant="secondary" size="icon" className="size-8 rounded-full bg-black/45 text-white hover:bg-black/65" aria-label="收起播放器" onClick={toggleExpanded}><Minimize2 className="size-4" /></Button><Button variant="secondary" size="icon" className="size-8 rounded-full bg-black/45 text-white hover:bg-black/65" aria-label="关闭播放器" onClick={closeMiniPlayer}><X className="size-4" /></Button></div>
            ...
            <div className="space-y-4 p-4"><div><input type="range" min={0} max={duration || 100} step="0.1" value={currentTime} aria-label="播放进度" onChange={(event) => seek(Number(event.target.value))} className="h-1.5 w-full accent-pink-500" /><div className="mt-1 flex justify-between text-xs text-muted-foreground"><span>{formatTime(currentTime)}</span><span>{formatTime(duration)}</span></div></div><div className="flex items-center justify-center gap-5"><Button variant="ghost" size="icon" aria-label="上一首" onClick={prevSong}><SkipBack className="size-5" /></Button><Button size="icon" className="size-13 rounded-full bg-pink-500 hover:bg-pink-600" aria-label={isPlaying ? "暂停" : "播放"} onClick={togglePlay}>{isPlaying ? <Pause className="size-5 fill-current" /> : <Play className="size-5 fill-current" />}</Button><Button variant="ghost" size="icon" aria-label="下一首" onClick={nextSong}><SkipForward className="size-5" /></Button></div><div className="flex items-center gap-2"><Button variant="ghost" size="icon" className="size-8" aria-label={isMuted ? "取消静音" : "静音"} onClick={toggleMute}>{isMuted ? <VolumeX className="size-4" /> : <Volume2 className="size-4" />}</Button><input type="range" min={0} max={100} value={isMuted ? 0 : volume * 100} aria-label="音量" onChange={(event) => setVolume(Number(event.target.value) / 100)} className="h-1.5 flex-1 accent-pink-500" /></div></div>
          </motion.aside>
        ) : (
          <motion.aside key="mini" initial={{ opacity: 0, y: 40, scale: .9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40, scale: .9 }} className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-full border bg-card px-3 py-2 shadow-xl" aria-label="迷你音乐播放器">
            ...
            <div className="relative flex size-10 items-center justify-center rounded-full" style={{ background: `conic-gradient(rgb(236 72 153) ${progress}%, hsl(var(--muted)) 0)` }}><Button variant="secondary" size="icon" className="size-8 rounded-full" aria-label={isPlaying ? "暂停" : "播放"} onClick={togglePlay}>{isPlaying ? <Pause className="size-3.5 fill-current" /> : <Play className="size-3.5 fill-current" />}</Button></div><Button variant="ghost" size="icon" className="size-8 rounded-full" aria-label="展开播放器" onClick={toggleExpanded}><Maximize2 className="size-4" /></Button><Button variant="ghost" size="icon" className="size-8 rounded-full" aria-label="关闭播放器" onClick={closeMiniPlayer}><X className="size-4" /></Button>
          </motion.aside>
        )}
      </AnimatePresence>
    </>
