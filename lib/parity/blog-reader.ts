...
// Astro orders category groups with Chinese collation, but orders entries
// inside each group with an ASCII-first path tie-breaker.
...
export function compareAstroCategoryPaths(a: string, b: string) {
  if (a === b) return 0;
...
export function compareAstroEntryPaths(a: string, b: string) {
  const numberA = leadingNumber(a);
  const numberB = leadingNumber(b);
  if (numberA !== numberB) return numberA - numberB;
...
export function sortBlogReaderPosts(posts: BlogReaderPost[]) {
  return [...posts].sort((a, b) => {
    const category = compareAstroCategoryPaths(a.categoryPath, b.categoryPath);
    return category || compareAstroEntryPaths(a.slug, b.slug);
  });
}
