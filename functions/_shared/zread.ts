...
export interface ZReadStructureItem {
  depth: number
  id: string
  title: string
  slug: string
...
export interface ZReadStructureResult {
  source: 'zread'
  sourceUrl: string
  items: ZReadStructureItem[]
}

export interface ZReadPageResult {
  source: 'zread'
  sourceUrl: string
  page: string
  slug: string
...
function repositoryUrl(owner: string, repo: string, slug = '') {
  const pathname = [owner, repo, slug]
    .filter(Boolean)
...
async function fetchZReadHtml(url: string) {
  ...
  try {
    ...
    if (!html || html.length > RESPONSE_LIMIT)
      throw new Error('ZRead 返回的文档体积异常')
    return html
  } finally {
...
function decodeFlightPayloads(html: string) {
  ...
  for (const match of html.matchAll(pattern)) {
    try {
      ...
      // A malformed analytics or framework payload is irrelevant; the wiki
      // metadata and Markdown live in their own independently decodable rows.
    }
  }
  return payloads
}

function extractPages(payloads: string[]) {
  for (const payload of payloads) {
    ...
    try {
      ...
      // Try the next flight row if this one was only a partial RSC reference.
    }
  }
...
function extractMarkdown(payloads: string[], slug: string) {
  const candidates = payloads
    ...
    .filter((entry): entry is { slug: string; markdown: string } =>
      Boolean(entry)
    )
  const selected =
...
function normalizeCallouts(markdown: string) {
  const callout = (input: string, tag: string, label: string): string =>
    input.replace(
...
function resolvePage(pages: ZReadPageMeta[], requested: string) {
  ...
  return pages.find(
    (page) =>
      normalizedKey(page.topic) === key || normalizedKey(page.slug) === key
  )
}

export async function fetchZReadStructure(
  owner: string,
  repo: string
): Promise<ZReadStructureResult> {
  const sourceUrl = repositoryUrl(owner, repo)
  const html = await fetchZReadHtml(sourceUrl)
  const pages = extractPages(decodeFlightPayloads(html)).sort(
...
export async function fetchZReadPage(
  owner: string,
  repo: string,
  requested = 'Overview'
): Promise<ZReadPageResult> {
  const repositorySourceUrl = repositoryUrl(owner, repo)
  const repositoryHtml = await fetchZReadHtml(repositorySourceUrl)
  const repositoryPayloads = decodeFlightPayloads(repositoryHtml)
  const pages = extractPages(repositoryPayloads).sort(
    (left, right) => left.order - right.order
  )
  ...
  const payloads =
    page === pages[0]
      ? repositoryPayloads
      : decodeFlightPayloads(await fetchZReadHtml(sourceUrl))
  const body = normalizeCallouts(extractMarkdown(payloads, page.slug))
  return {
    source: 'zread',
    sourceUrl,
    page: page.topic,
