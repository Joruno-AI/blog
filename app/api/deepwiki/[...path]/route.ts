...
import { fetchDeepWikiPage, fetchDeepWikiStructure } from "@/lib/agent/deepwiki";
...
export async function GET(request: Request, context: Context) {
  return cachedAgentResponse(request, async () => {
    ...
    try {
      if (action === "structure") return json(await fetchDeepWikiStructure(owner, repo));
      if (action === "overview" || action === "page") {
        ...
        return json(await fetchDeepWikiPage(owner, repo, requested));
      }
      return json({ error: "不支持的 DeepWiki 操作。" }, 404, "no-store");
    } catch (reason) {
      const timedOut = reason instanceof Error && reason.name === "AbortError";
      return json({ error: timedOut ? "DeepWiki 生成超时，请稍后重试。" : reason instanceof Error ? reason.message : "DeepWiki 文档加载失败。" }, 502, "no-store");
    }
  });
}
